...
module.exports ={
            ...
            connection.query('SELECT * FROM catatan pupuk;', function (error, results) {
                ...
                if (results.length > 0) {
                    res.render('CatatanPupuk', {
                        ...
                        catatanPupuk: results // Pass the CatatanPupuk data to the view
                    });
                } else {
                    res.render('CatatanPupuk', {
                        url: 'http://localhost:3000/',
    ...
    editCatatanPupuk(req, res) {
        ...
        pool.getConnection(function (err, connection) {
            if (err) throw err;
            connection.query('SELECT * FROM CatatanPupuk WHERE id = ?', [id], function (error, results) {
                if (error) throw error;
                if (results.length > 0) {
    ...
    updateCatatanPupuk(req, res) {
        ...
        pool.getConnection(function (err, connection) {
            if (err) throw err;
            connection.query(
                'UPDATE CatatanPupuk SET namaPetani = ?, lokasiLahan = ?, luasLahan = ?, jenisTanaman = ? WHERE id = ?',
    ...
    deleteCatatanPupuk(req, res) {
        ...
        pool.getConnection(function (err, connection) {
            if (err) throw err;
            connection.query('DELETE FROM CatatanPupuk WHERE id = ?', [id], function (error, results) {
                if (error) throw error;
                res.redirect('/CatatanPupuk');
            });
            connection.release();
